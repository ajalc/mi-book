...
**Description**: To meet demands for high-k dielectrics, we must train a machine learning model to predict the total dielectric contribution for elements in the Material Project that don't already have dielectric constants. With the use of matminer features, we can observe linear regressions and cross validate our data, to get a relatively low error. This means that after minimizing the features and properties, our models get better predictions at the end of the data analysis. After gathering promising elements, we are ready to consider their sustainability, requiring more research to finalize our list. After considering cost and performance, the materials are ready to be scientifically experimented.
